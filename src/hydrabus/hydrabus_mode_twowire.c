...
void twowire_init_proto_default(t_hydra_console *con)
{
	...
	proto->config.rawwire.clk_pin = 10;
	proto->config.rawwire.sdi_pin = 2;
}
...
bool twowire_pin_init(t_hydra_console *con)
{
	...
	bsp_gpio_init(BSP_GPIO_PORTA, proto->config.rawwire.clk_pin,
		      proto->config.rawwire.dev_gpio_mode, proto->config.rawwire.dev_gpio_pull);
	bsp_gpio_init(BSP_GPIO_PORTA, proto->config.rawwire.sdi_pin,
		      proto->config.rawwire.dev_gpio_mode, proto->config.rawwire.dev_gpio_pull);
	return true;
}
...
{
	mode_config_proto_t* proto = &con->mode->proto;
	bsp_gpio_mode_in(BSP_GPIO_PORTA, proto->config.rawwire.sdi_pin);
}

static void twowire_sda_mode_output(t_hydra_console *con)
{
	mode_config_proto_t* proto = &con->mode->proto;
	bsp_gpio_mode_out(BSP_GPIO_PORTA, proto->config.rawwire.sdi_pin);
}

inline void twowire_sda_high(t_hydra_console *con)
{
	mode_config_proto_t* proto = &con->mode->proto;
	bsp_gpio_set(BSP_GPIO_PORTA, proto->config.rawwire.sdi_pin);
}

inline void twowire_sda_low(t_hydra_console *con)
{
	mode_config_proto_t* proto = &con->mode->proto;
	bsp_gpio_clr(BSP_GPIO_PORTA, proto->config.rawwire.sdi_pin);
}
...
inline void twowire_clk_high(t_hydra_console *con)
{
	...
	bsp_tim_wait_irq();
	bsp_gpio_set(BSP_GPIO_PORTA, proto->config.rawwire.clk_pin);
	bsp_tim_clr_irq();
}
...
inline void twowire_clk_low(t_hydra_console *con)
{
	...
	bsp_tim_wait_irq();
	bsp_gpio_clr(BSP_GPIO_PORTA, proto->config.rawwire.clk_pin);
	bsp_tim_clr_irq();
}
...
uint8_t twowire_read_bit(t_hydra_console *con)
{
	...
	return bsp_gpio_pin_read(BSP_GPIO_PORTA, proto->config.rawwire.sdi_pin);
}
...
uint8_t twowire_read_bit_clock(t_hydra_console *con)
{
	...
	if (proto->config.rawwire.clock_polarity == 0) {
		twowire_clk_high(con);
		bit = bsp_gpio_pin_read(BSP_GPIO_PORTA, proto->config.rawwire.sdi_pin);
		twowire_clk_low(con);
	} else {
		twowire_clk_low(con);
		bit = bsp_gpio_pin_read(BSP_GPIO_PORTA, proto->config.rawwire.sdi_pin);
